...
public class AddMeasurementGroupChannelDialog extends JDialog implements IStringSpectrumListener
{
	...
	public AddMeasurementGroupChannelDialog(DevicePool devicePool, MeasurementGroup measurementGroup)
	{
		super();
		...
		setTitle("Add channel to " + measurementGroup.getName());
	...
	protected void addPressed()
	{
		...
		for(Object item : availableChannels.getSelectedValues())
		{
			try 
			{
				measurementGroup.addNewExpChannel((ExperimentChannel) item);
			} 
		...
		if(failed.size() > 0)
		{
			...
			JOptionPane.showMessageDialog(this, 
					buff.toString(),
					"Error trying to add channel(s) to a measurement group", 
	...
	protected void updateChannels()
	{
		...
		List<ExperimentChannel> availableChannelList = 
			new ArrayList<ExperimentChannel>(devicePool.getExperimentChannels());

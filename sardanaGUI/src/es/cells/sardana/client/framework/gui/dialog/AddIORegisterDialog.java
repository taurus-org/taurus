...
public class AddIORegisterDialog extends JDialog 
{
	...
	public AddIORegisterDialog()
	{
		super();
		setTitle("Create IORegister");
	}
			
	public AddIORegisterDialog(DevicePool devicePool)
	{
		this();
	...
	public AddIORegisterDialog(DevicePool devicePool, IORegister channel)
	{
		this();
		this.devicePool = devicePool;
	...
	void updateControllers()
	{
		...
		for(Controller ctrl : devicePool.getControllers() )
			if(ctrl.getType() == ControllerType.IORegister)
	...
	protected void init()
	{
		...
		createButton.setToolTipText("Create a new ioregister");
	...
	protected void createPressed(ActionEvent e)
	{
		...
		try
		{
			...
			args.insert(params);
			devicePool.getDevice().executeCommand(DevicePoolUtils.POOL_CMD_CREATE_IOREGISTER,args);
			
			JOptionPane.showMessageDialog(this,
					"IORegister " + name + " sucessfully created","Sucess!", JOptionPane.INFORMATION_MESSAGE);

		}
		catch (DevFailed devFailed)
		{
			log.fine("User error trying to create ioregister with: " + ctrl.getName() + ", " + index + ", " + name);
			...
			JOptionPane.showMessageDialog(this, 
					buff.toString(),
					"Error trying to create a new ioregister", 
	...
	class ChannelNumberModel extends SpinnerNumberModel implements ActionListener 
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				IORegister ioRegister = (IORegister)elem;
				invalidChannelNumbers[i++] = ioRegister.getIdInController();
			}

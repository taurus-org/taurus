...
// file :         TwoDThread.cpp
//
// description :  C++ source for the TwoDThread class. 
//
...
//-----------------------------------------------------------------------------
//
// method : 		TwoDThread::run_undetached
// 
// description : 	The thread main code
//
//-----------------------------------------------------------------------------
    
void *TwoDThread::run_undetached(void *ptr)
{
    AutoCleanPythonGIL auto_clean_gil;
    
    DEBUG_STREAM << "The TwoD thread starts..." << endl;
...
//
    ...
    TwoDController *typed_ctrl = static_cast<TwoDController *>(the_dev->get_controller());
            ...
            {
                {
                    ...
                    the_shared_data.errors[0].origin = CORBA::string_dup("TwoDThread::run_undetached()");
        ...
        {
            {
                ...
                the_shared_data.errors[0].severity = Tango::ERR;
                the_shared_data.errors[0].origin = CORBA::string_dup("TwoDThread::run_undetached()");
                the_shared_data.errors[0].reason = CORBA::string_dup("TwoDExpChannel_BadController");
...
void TwoDThread::th_exit(Tango::Attribute &the_att,bool fire_last_event)
{
...
//-----------------------------------------------------------------------------
//
// method : 		TwoDThread::is_enough_time
// 
...
bool TwoDThread::is_enough_time(long needed,struct timeval &stop,long &remain_time)
{
    bool ret_value;

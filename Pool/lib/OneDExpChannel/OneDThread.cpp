...
// file :         OneDThread.cpp
//
// description :  C++ source for the OneDThread class. 
//
...
//-----------------------------------------------------------------------------
//
// method : 		OneDThread::run_undetached
// 
// description : 	The thread main code
//
//-----------------------------------------------------------------------------
    
void *OneDThread::run_undetached(void *ptr)
{
    AutoCleanPythonGIL auto_clean_gil;
        
    DEBUG_STREAM << "The OneD thread starts..." << endl;

//
// Some initialisation
//

    long local_ct_idx = the_dev->get_axis();
    OneDController *typed_ctrl = static_cast<OneDController *>(the_dev->get_controller());
...
//
        ...
        
        {
            ...
            try
            {
                ...
                typed_ctrl->ReadAll();
                //Teresa	tmp_read_value = typed_ctrl->ReadOne(local_ct_idx);
            }
            ...
            {
                {
                    ...
                    the_shared_data.errors[0].origin = CORBA::string_dup("OneDThread::run_undetached()");
        ...
        {
            {
                ...
                the_shared_data.errors[0].severity = Tango::ERR;
                the_shared_data.errors[0].origin = CORBA::string_dup("OneDThread::run_undetached()");
                the_shared_data.errors[0].reason = CORBA::string_dup("OneDExpChannel_BadController");
...
void OneDThread::th_exit(Tango::Attribute &the_att,bool fire_last_event)
{
...
//-----------------------------------------------------------------------------
//
// method : 		OneDThread::is_enough_time
// 
...
bool OneDThread::is_enough_time(long needed,struct timeval &stop,long &remain_time)
{
    bool ret_value;

...
from taurus.qt.qtcore.model import TaurusTreeBaseItem, TaurusBaseModel
...
class MacroTreeBaseItem(TaurusTreeBaseItem):
    """A generic node"""
...
class MacroTreeModuleItem(MacroTreeBaseItem):
...
class MacroTreeMacroItem(MacroTreeBaseItem):
...
class MacroBaseModel(TaurusBaseModel):
    ...
    def setDataSource(self, ms):
        if self._data_src is not None:
            Qt.QObject.disconnect(self._data_src, Qt.SIGNAL('macrosUpdated'), self.refresh)
        TaurusBaseModel.setDataSource(self, ms)
        if ms is not None:
            Qt.QObject.connect(ms, Qt.SIGNAL('macrosUpdated'), self.refresh)
...
class MacroPlainMacroModel(MacroBaseModel):
    ...
    def setupModelData(self, data):
        ...
        root = self._rootItem
        for macro_name, macro in ms.getMacros().items():
            macroNode = MacroTreeMacroItem(self, macro, root)
            root.appendChild(macroNode)


class MacroBaseModelProxy(Qt.QSortFilterProxyModel):
    ...
    def __init__(self, parent=None):
        ...
        self.setFilterCaseSensitivity(Qt.Qt.CaseSensitive)

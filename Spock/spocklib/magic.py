...
def _get_macro_server():
    ...
    return spocklib.genutils.get_macro_server()
    
def _get_door():
    import spocklib.genutils
    return spocklib.genutils.get_door()
...
def edmac(self, parameter_s=''):
    ...
    import spocklib.genutils
    pars = spocklib.genutils.arg_split(parameter_s, posix=True)
    ...
    if spocklib.genutils.ask_yes_no('Do you want to apply the new code on the server?', 'y'):
        print 'Storing...',
        try:
            f = file(local_fname)
            try:
                ...
                print spocklib.genutils.MSG_DONE
            except Exception, e:
                print spocklib.genutils.MSG_FAILED
                print 'Reason:', str(e)
            f.close()
        except:
    ...
    #    if spocklib.genutils.ask_yes_no('Delete temporary file \'%s\'?' % local_fname, 'y'):
...
def spock_input_prompt_hook(self, cont):
    try:
        ...
        return
        import spocklib
        return spocklib.Door.spock_offline_input_prompt(self.api, cont)
...
def spock_output_prompt_hook(self):
    try:
        ...
        return
        import spocklib
        return spocklib.Door.spock_offline_output_prompt(self.api)
